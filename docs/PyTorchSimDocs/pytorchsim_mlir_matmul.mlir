...
func.func @kernel(%X: memref<1048576xf32>, %W: memref<1048576xf32>, %Y: memref<1048576xf32>) {
  ...
  %t_const2 = arith.constant 2 : index
  %t_const3 = arith.constant 3 : index
  ...
  affine.for %index0 = 0 to 1024 step 1024 {
    affine.for %index1 = 0 to 1024 step 1024 {
      ...
      affine.for %index2 = 0 to 1024 step 256 {
        ...
        memref.dma_start %X[%apply0], %X_buffer[%t_const0, %t_const0], %t_const2, %t_alloc0[%t_const0], %t_const1, %t_const1 : memref<1048576xf32>, memref<1024x256xf32, 1>, memref<1xi32> 
          {dram_stride=[1024, 1], sram_stride=[1, 1024], padding=0, subtile_size=[128, 256], async=1}
        %apply1 = affine.apply #t_map0(%index2, %index1)[]
        memref.dma_start %W[%apply1], %W_buffer[%t_const0, %t_const0], %t_const2, %t_alloc1[%t_const0], %t_const1, %t_const1 : memref<1048576xf32>, memref<256x1024xf32, 1>, memref<1xi32> 
          {dram_stride=[1024, 1], sram_stride=[1, 256], padding=0, subtile_size=[256, 1024], async=1}
      ...
      affine.for %compute_idx = 0 to 8192 step 64
      {
      } {inner_loop=false}
      %epilogue_apply0 = affine.apply #t_map0(%index0, %index1)[]
      memref.dma_start %Y_buffer[%t_const0, %t_const0], %Y[%epilogue_apply0], %t_const3, %t_alloc2[%t_const0], %t_const1, %t_const1 : memref<1024x1024xf32, 1>, memref<1048576xf32>, memref<1xi32> 
        {dram_stride=[1024, 1], sram_stride=[1, 1024], padding=0}
