...
#include "my_device/src/device/tpu_sim/tpu_sim_device_plugin.h"
...
void* plugin_unified_memory_allocate(const SP_Device* device, uint64_t bytes) {
  return aligned_alloc(64, bytes);
}

void plugin_unified_memory_deallocate(const SP_Device* device, void* location) {
  free(location);
}
...
void plugin_memset(const SP_Device* device, SP_Stream stream,
                   ...
                   uint64_t size, TF_Status* status) {
  memset(location->opaque, pattern, size);
}
...
void plugin_memset32(const SP_Device* device, SP_Stream stream,
                     ...
                     uint64_t size, TF_Status* status) {
  uint32_t* ptr = static_cast<uint32_t*>(location->opaque);
...
void plugin_create_stream_executor(const SP_Platform* platform,
  ...
  params->stream_executor->unified_memory_allocate =
      plugin_unified_memory_allocate;
  params->stream_executor->unified_memory_deallocate =
      plugin_unified_memory_deallocate;
