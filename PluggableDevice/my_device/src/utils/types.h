...
#ifndef TENSORFLOW_PLUGIN_SRC_UTILS_TYPES_H_
#define TENSORFLOW_PLUGIN_SRC_UTILS_TYPES_H_
...
// clang-format on
#include "my_device/src/utils/types.pb.h"
//#include "my_device/src/utils/numeric_types.h"
#include "my_device/src/utils/integral_types.h"
#include "my_device/src/utils/gtl/array_slice.h"
#include "my_device/src/utils/gtl/inlined_vector.h"
#include "my_device/src/utils/logging.h"
#include "my_device/src/utils/stringpiece.h"
#include "my_device/src/utils/tstring.h"
...
template <typename Device> struct DeviceName {};
/*
template <> struct DeviceName<Eigen::ThreadPoolDevice> {
  static const std::string value;
};

template <> struct DeviceName<Eigen::GpuDevice> {
  static const std::string value;
};
*/
...
template <class T> struct IsValidDataType;
...
template <DataType VALUE> struct EnumToDataType {}; // Specializations below

// Template specialization for both DataTypeToEnum and EnumToDataType.
#define MATCH_TYPE_AND_ENUM(TYPE, ENUM)                                        \
  template <> struct DataTypeToEnum<TYPE> {                                    \
    static DataType v() { return ENUM; }                                       \
    static DataType ref() { return MakeRefType(ENUM); }                        \
    static constexpr DataType value = ENUM;                                    \
  };                                                                           \
  template <> struct IsValidDataType<TYPE> {                                   \
    static constexpr bool value = true;                                        \
  };                                                                           \
  template <> struct EnumToDataType<ENUM> { typedef TYPE Type; }
...
template <> struct DataTypeToEnum<long> {
...
template <> struct IsValidDataType<long> {
  static constexpr bool value = true;
};
template <> struct EnumToDataType<DT_INT64> {
  typedef demo_plugin::int64 Type;
};

template <> struct DataTypeToEnum<unsigned long> {
  static DataType v() { return value; }
  static DataType ref() { return MakeRefType(value); }
  static constexpr DataType value =
      sizeof(unsigned long) == 4 ? DT_UINT32 : DT_UINT64;
};
template <> struct IsValidDataType<unsigned long> {
  static constexpr bool value = true;
};
template <> struct EnumToDataType<DT_UINT64> {
  typedef demo_plugin::uint64 Type;
};

template <> struct DataTypeToEnum<long long> {
  static DataType v() { return DT_INT64; }
  static DataType ref() { return MakeRefType(DT_INT64); }
  static constexpr DataType value = DT_INT64;
};
template <> struct IsValidDataType<long long> {
  static constexpr bool value = true;
};

template <> struct DataTypeToEnum<unsigned long long> {
...
template <> struct IsValidDataType<unsigned long long> {
  static constexpr bool value = true;
};
...
#endif // TAENSORFLOW_PLUGIN_SRC_UTILS_TYPES_H_
