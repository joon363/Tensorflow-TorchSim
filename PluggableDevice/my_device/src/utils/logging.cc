...
#include "my_device/src/utils/env_time.h"
...
#else
  // Read TENSORFLOW_PLUGIN env var first.
  const char *tf_env_var_val = getenv("TF_CPP_MIN_LOG_LEVEL");
  if (tf_env_var_val == nullptr)
    tf_env_var_val = getenv("TF_CPP_MIN_LOG_LEVEL");
...
int64 MinVLogLevelFromEnv() {
  ...
  if (tf_env_var_val == nullptr)
    tf_env_var_val = getenv("TF_CPP_MAX_VLOG_LEVEL");
  return LogLevelStrToInt(tf_env_var_val);
#endif
}
