...
- TensorFlow의 모델을 실행하되, **Eager Mode(인터프리터 방식)가 아닌 Graph Mode**로 실행하여 시뮬레이터의 성능(Fusion, Tiling)을 극대화한다. 또한 기존 Python 백엔드 코드를 재작성하지 않고 재사용한다.

### 3.2 본 문서의 목표
- 아주 간단한 1*2 벡터의 덧셈 연산에 대해 아래 결과를 얻을 수 있었다. 
    1) torch 모델을 torchax를 사용해 StableHLO로 변환한 결과
    2) Tensorflow의 모델을 XLA를 사용해 StableHLO로 변환한 결과
        2.1) stablehlo-opt를 사용해 --stablehlo-legalize-to-linalg, --linalg-specialize-generic-ops를 처리하고 mlir-opt를 사용해 bufferize,-convert-linalg-to-loops를 처리한 결과
        - 자세한 내용은 test_lowering.ipynb에 있다.
    3) PyTorchSim의 mlir_codegen_backend.py의 codegen_loops의 Hand-Written MLIR Kernel이 생성하는 MLIR 코드.
        - 이때 PyTorchSim이 생성하는 MLIR 코드는 PyTorch를 XLA 컴파일러를 사용해 만든 MLIR 코드와는 다르다. 
- 첫번째 목표는 각 단계의 특징을 분석하고, 추상화 단계와 품고 있는 정보 (차원 등)이 어떤 것이 있는지 최대한 자세하게 보는 것이다.
- 두번째 목표는 자동으로 만들어진 tensorflow->stableHLO 코드를 어떻게 PyTorchSim이 만들어내는 MLIR의 형식으로 변환해야 하는지, 어떤 정보가 빠져 있고 구조는 어떻게 바꾸어야 하는지 한줄한줄 자세히 분석하는 것이다.
...
**StableHLO는 다음을 제공하지 않음:**
1. **루프 구조** - 연산만 있고, 타일링/반복 정보 없음
2. **메모리 계층** - 메모리 공간(DRAM/SPAD) 구분 없음
3. **DMA 명시** - 데이터 이동이 암시적 (컴파일러가 삽입해야 함)
4. **벡터화 정보** - Element-wise 연산으로만 표현
5. **하드웨어 특성** - NPU/ASIC의 타일링, 병렬화 전략이 없음

### 6.2 PyTorchSim의 추가 정보
...
### 7.1 StableHLO → PyTorchSim MLIR 변환 모듈 설계

**입력:** StableHLO MLIR  
**출력:** PyTorchSim Hand-Written MLIR

```
StableHLO Input:
  func @main(tensor<2xf32>, tensor<2xf32>) -> tensor<2xf32> {
    stablehlo.add ...
  }
```
↓↓↓
...
**필요한 분석 & 생성:**
1. input, output 정의
2. codegen_loops에 해당하도록 바꾸어야 함.
  
    2.1 현재 PyTorchSim은 torch FX Graph의 Node Type에 따라, 그리고 call_function인 경우 ExtensionOpOverride를 따라 MLIR 코드를 생성하고 있음. StableHLO 코드와 현재 PyTorchSim Codegen Engine의 연결을 어떻게 할 것인지 깊은 생각과 논의가 필요함.
  
  
    2.2 만약 Incompatible하다고 판단될 경우 현재 구조를 바탕으로, StableHLO 기준으로 다시 Machine-Specific 정보 (6.2에서 명시한)를 추가한 MLIR 코드를 만드는 패스를 작성하는 방법도 가능함.


↓↓↓

PyTorchSim MLIR Output:
```
  func @kernel(memref<2xf32>, memref<2xf32>, memref<2xf32>) {
    affine.for ... {
      memref.dma_start (load)
      affine.for ... {
        affine.vector_load/store
        arith.addf (vector)
      }
      memref.dma_start (store)
    }
  }
```


### 7.2 참고: mlir_codegen_backend.py의 `codegen_loops()` 함수

CodegenResearch.md 참조:
- 이미 PyTorchSim이 보유한 루프 생성 로직
- `LoopLevel`, `LoopNest` 클래스로 추상화
- `ParallelLoopBuffer`에 코드 누적
- **이 구조를 StableHLO 입력에도 적용 가능**
